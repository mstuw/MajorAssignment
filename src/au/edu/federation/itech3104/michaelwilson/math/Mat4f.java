 ...
 * <p>
 * The Mat4f class contains 16 public member properties m00 through m15 which
 * are <strong>column major</strong>, so our matrix looks like this:
 * <p>
 * m00 m10 m20 m30
 * <p>
 * m01 m11 m21 m31
...
public class Mat4f {
	...
	public Mat4f(float value) {
		...
		// All other values are implicitly set to zero by Java as that is the default
		// value of a float primitive.
	}
	...
	/**
	 * Returns a new scale transformation matrix.
	 */
	public static Mat4f scaleMatrix(float x, float y, float z) {
		Mat4f scaleT = new Mat4f(1.0f);
		scaleT.m00 = x;
		scaleT.m11 = y;
		scaleT.m22 = z;
		return scaleT;
	 ...
	 * Returns a new scale transformation matrix.
	 */
	public static Mat4f scaleMatrix(Vec3f vec) {
	...
	public static Mat4f lookAt(Vec3f position, Vec3f target, Vec3f up) {

		Vec3f zaxis = position.minus(target).normalise();
		...
		Mat4f translation = new Mat4f(1.0f);
	 ...
	 * want to use the {@link Vec3f} class.
	 * 
	 ...
	 * properties.
	 * <p>
	 * To ensure the legality of the x-basis, the provided Vec3f is normalised if
	 * required before being set.
	 * <p>
	 ...
	 * {@link #setFromArray(float[])} and {@link #Mat4f(float[])}.
	 * 
	 ...
	 * Vec3f and normalised (if required) before being set.
	 * <p>
	 * If you wish to use this class for storing matrices which do not represent a
	 * rotation matrix then you should avoid the setXBasis/setYBasis/setZBasis
	 ...
	 * properties.
	 * <p>
	 * To ensure the legality of the y-basis, the provided Vec3f is normalised if
	 * required before being set.
	 * <p>
	 * If you wish to use this class for storing matrices which do not represent a
	 * rotation matrix then you should avoid the setXBasis/setYBasis/setZBasis
	 ...
	 * properties.
	 * <p>
	 * To ensure the legality of the z-basis, the provided Vec3f is normalised if
	 * required before being set. If you wish to use this class for storing matrices
	 * which do not represent a rotation matrix then you should avoid the
	 * setXBasis/setYBasis/setZBasis methods and instead set the matrix properties
	 * via other methods which do not enforce normalisation such as the
	 ...
	 * 
	 * @return A Vec3f of the origin location of this Mat4f, as stored in the m30,
	 *         m31 and m32 properties.
	 */
	...
	/**
	 * Return whether or not all three axes of this Mat4f are orthogonal (i.e. at 90
	 ...
	 * <p>
	 * If you want to return a transposed version of a Mat4f <em>without
	 ...
	 * <p>
	 * If you want to transpose 'this' Mat4f rather than return a transposed copy
	 * then use the {@link #transpose()} method instead.
	 * 
	 * @return A transposed version of this Mat4f.
	 */
	 ...
	 * multiplication process, instead a new Mat4f containing the resulting matrix
	 * is returned.
	 * <p>
	 ...
	 * @return A Mat4f which is the result of multiplying this matrix by the
	 *         provided matrix.
	 */
	 ...
	 * <p>
	 * This method multiplies the provided Vec3f by this matrix. This is commonly
	 * used to transform vertices between coordinate spaces, for example you might
	 * multiply a vertex location by a model matrix to transform the vertex from
	 * model space into world space.
	 * <p>
	 * The difference between this method and the {@link #transformDirection} method
	 * is that this method takes the origin of this Mat4f into account when
	...
	public Vec3f transformPoint(Vec3f v) {
		...
		result.x = this.m00 * v.x + this.m10 * v.y + this.m20 * v.z + this.m30; // * 1.0f; - no need as w is 1.0 for a
																				// location
		result.y = this.m01 * v.x + this.m11 * v.y + this.m21 * v.z + this.m31; // * 1.0f; - no need as w is 1.0 for a
																				// location
		result.z = this.m02 * v.x + this.m12 * v.y + this.m22 * v.z + this.m32; // * 1.0f; - no need as w is 1.0 for a
																				// location
	 ...
	 * that this method does not take the origin of this Mat4f into account when
	 * performing the transformation, whilst transformPoint does.
	 * 
	...
	public Vec3f transformDirection(Vec3f v) {
		...
		// NO!
		result.x = this.m00 * v.x + this.m10 * v.y + this.m20 * v.z; // + this.m30 * 0.0; - no need as w is 0.0 for a
																		// direction
		result.y = this.m01 * v.x + this.m11 * v.y + this.m21 * v.z; // + this.m31 * 0.0; - no need as w is 0.0 for a
																		// direction
		result.z = this.m02 * v.x + this.m12 * v.y + this.m22 * v.z; // + this.m32 * 0.0; - no need as w is 0.0 for a
																		// direction
	...
	public static Mat4f inverse(Mat4f m) {
		...
		temp.m00 = m.m12 * m.m23 * m.m31 - m.m13 * m.m22 * m.m31 + m.m13 * m.m21 * m.m32 - m.m11 * m.m23 * m.m32 - m.m12 * m.m21 * m.m33
				+ m.m11 * m.m22 * m.m33;
		temp.m01 = m.m03 * m.m22 * m.m31 - m.m02 * m.m23 * m.m31 - m.m03 * m.m21 * m.m32 + m.m01 * m.m23 * m.m32 + m.m02 * m.m21 * m.m33
				- m.m01 * m.m22 * m.m33;
		temp.m02 = m.m02 * m.m13 * m.m31 - m.m03 * m.m12 * m.m31 + m.m03 * m.m11 * m.m32 - m.m01 * m.m13 * m.m32 - m.m02 * m.m11 * m.m33
				+ m.m01 * m.m12 * m.m33;
		temp.m03 = m.m03 * m.m12 * m.m21 - m.m02 * m.m13 * m.m21 - m.m03 * m.m11 * m.m22 + m.m01 * m.m13 * m.m22 + m.m02 * m.m11 * m.m23
				- m.m01 * m.m12 * m.m23;
		temp.m10 = m.m13 * m.m22 * m.m30 - m.m12 * m.m23 * m.m30 - m.m13 * m.m20 * m.m32 + m.m10 * m.m23 * m.m32 + m.m12 * m.m20 * m.m33
				- m.m10 * m.m22 * m.m33;
		temp.m11 = m.m02 * m.m23 * m.m30 - m.m03 * m.m22 * m.m30 + m.m03 * m.m20 * m.m32 - m.m00 * m.m23 * m.m32 - m.m02 * m.m20 * m.m33
				+ m.m00 * m.m22 * m.m33;
		temp.m12 = m.m03 * m.m12 * m.m30 - m.m02 * m.m13 * m.m30 - m.m03 * m.m10 * m.m32 + m.m00 * m.m13 * m.m32 + m.m02 * m.m10 * m.m33
				- m.m00 * m.m12 * m.m33;
		temp.m13 = m.m02 * m.m13 * m.m20 - m.m03 * m.m12 * m.m20 + m.m03 * m.m10 * m.m22 - m.m00 * m.m13 * m.m22 - m.m02 * m.m10 * m.m23
				+ m.m00 * m.m12 * m.m23;
		temp.m20 = m.m11 * m.m23 * m.m30 - m.m13 * m.m21 * m.m30 + m.m13 * m.m20 * m.m31 - m.m10 * m.m23 * m.m31 - m.m11 * m.m20 * m.m33
				+ m.m10 * m.m21 * m.m33;
		temp.m21 = m.m03 * m.m21 * m.m30 - m.m01 * m.m23 * m.m30 - m.m03 * m.m20 * m.m31 + m.m00 * m.m23 * m.m31 + m.m01 * m.m20 * m.m33
				- m.m00 * m.m21 * m.m33;
		temp.m22 = m.m01 * m.m13 * m.m30 - m.m03 * m.m11 * m.m30 + m.m03 * m.m10 * m.m31 - m.m00 * m.m13 * m.m31 - m.m01 * m.m10 * m.m33
				+ m.m00 * m.m11 * m.m33;
		temp.m23 = m.m03 * m.m11 * m.m20 - m.m01 * m.m13 * m.m20 - m.m03 * m.m10 * m.m21 + m.m00 * m.m13 * m.m21 + m.m01 * m.m10 * m.m23
				- m.m00 * m.m11 * m.m23;
		temp.m30 = m.m12 * m.m21 * m.m30 - m.m11 * m.m22 * m.m30 - m.m12 * m.m20 * m.m31 + m.m10 * m.m22 * m.m31 + m.m11 * m.m20 * m.m32
				- m.m10 * m.m21 * m.m32;
		temp.m31 = m.m01 * m.m22 * m.m30 - m.m02 * m.m21 * m.m30 + m.m02 * m.m20 * m.m31 - m.m00 * m.m22 * m.m31 - m.m01 * m.m20 * m.m32
				+ m.m00 * m.m21 * m.m32;
		temp.m32 = m.m02 * m.m11 * m.m30 - m.m01 * m.m12 * m.m30 - m.m02 * m.m10 * m.m31 + m.m00 * m.m12 * m.m31 + m.m01 * m.m10 * m.m32
				- m.m00 * m.m11 * m.m32;
		temp.m33 = m.m01 * m.m12 * m.m20 - m.m02 * m.m11 * m.m20 + m.m02 * m.m10 * m.m21 - m.m00 * m.m12 * m.m21 - m.m01 * m.m10 * m.m22
				+ m.m00 * m.m11 * m.m22;
	...
	/*
	 * public static Mat4f rotateAroundArbitraryAxisRads(float angleRads, Vec3f
	 * axis) { Mat4f result = new Mat4f();
	 * 
	...
	public Mat4f rotateAboutLocalAxisRads(float angleRads, Vec3f localAxis) {
		localAxis = localAxis.normalised();
	...
	// This either does NOT work properly, or it is not the right tool for the job
	// to rotate
	// the camera matrix! If you use this to rotate the camera matrix the entire
	// fucking thing
	 ...
	 * provided matrix. That is, if the positive X-axis (that is, the axis
	 * {@code Vec3f(1.0f, 0.0f, 0.0f)}) is specified as the axis of rotation, then
	 * the positive X-axis of the provided rotation matrix is used as the rotation
	 * matrix, which is not necessarily (or in fact likely to be) the same direction
	 * as the global 'world-space' X-axis.
	 * 
	 * @param matrix    The matrix to rotate.
	 ...
	 * <p>
	 * The x, y and z axes are returned in the Mat3f, while the w component of each
	 * axis and the origin plus it's w component are discarded.
	 * <p>
	...
	public static Mat4f createPerspectiveProjectionMatrix(float vertFoVDegs, float aspectRatio, float zNear, float zFar) {
		// Sanity check the aspect ratio and throw an illegal argument exception if it
		// is invalid
		if (aspectRatio < 0.0f) {
		...
		// Calculate the frustum length and throw an illegal argument exception if it is
		// invalid
		...
		// Sanity check the vertical field of view and throw an illegal argument
